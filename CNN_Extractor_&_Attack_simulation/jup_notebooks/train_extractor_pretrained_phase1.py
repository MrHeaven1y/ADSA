...
class SegmentTransform:
    ...
    def __call__(self, img, mask):
        ...
        if random.random() < self.p:
            ...
            mask = TF.resized_crop(mask, i, j, h, w, self.image_size, interpolation=TF.InterpolationMode.NEAREST)
...
def get_mode_weights_phase1(epoch, max_epochs):
    """Phase 1: only modes 0, 3, 4. No tamper, no fake."""
    # [mode0=attacked, mode1=tamper, mode2=fake, mode3=clean_wm, mode4=no_wm]
    return torch.tensor([0.60, 0.00, 0.00, 0.20, 0.20])
...
class ForensicInfoNCELoss(nn.Module):
    ...
    def forward(self, pred_int, gt_int, pred_glob, gt_glob, z_mean, z_logvar, 
            gt_latent, pred_fp, identity_labels, extractor, 
            current_epoch=0, latent_weight=1.0, id_weight=1.0, 
            collapse_weight=0.1, max_epochs=80, mode="full"):  # <-- ADD mode param
        ...
        if mode == "global_only":
            ...
            return loss_global, {
                'loss_ext_total': loss_global.item(),
        ...
        # ─────────────────────────────────────────────────────────────
        # PHASE 3: FULL (your existing code below, unchanged)
        # ─────────────────────────────────────────────────────────────
        elif mode == "full":
            # ... your existing massive loss calculation stays here ... 
            loss_spatial_bce = self.bce(pred_int, gt_int)
        ...
        for step, (images, masks) in enumerate(train_loader):
            ...
            with sync_ctx:
                with autocast(device_type='cuda'):
                    ...
                    # ═══════════════════════════════════════════════════════════════════════
                    if PHASE == "global_only":
                        mode_weights = get_mode_weights_phase1(itr, max_iterations).to(rank)
                    else:
                    ...
                    if attack_mode == 0:
                        ...
                        # Fallback – if mode 1/2 accidentally selected in Phase 1, treat as mode 0
                        wm_attacked = attack_layer(wm_composite, itr, max_epochs=max_iterations)
                        gt_glob[:] = 0
                    ...
                    if PHASE == "global_only":
                        gt_latent = torch.zeros_like(z_mean)
                        identity_ids = torch.zeros(B, dtype=torch.long, device=rank)
                        ...
                        loss, bd = criterion(
                            ...
                            max_epochs=max_iterations, mode="global_only"
                        )
        ...
        with torch.inference_mode():
            for step, (images, masks) in enumerate(val_loader):
                ...
                with autocast(device_type='cuda'):
                    ...
                    if PHASE == "global_only":
                        val_mode_weights = get_mode_weights_phase1(itr, max_iterations).to(rank)
                    else:
                        val_mode_weights = get_mode_weights(itr, max_iterations).to(rank)
                    attack_mode = torch.multinomial(
                        val_mode_weights, 1, generator=torch.Generator(device=rank).manual_seed(step)
                    ).item()
                    ...
                    if PHASE == "global_only":
                        # Dummy latent – won’t be used by the loss, just to match signature
                        gt_latent = torch.zeros(B, 256, 28, 28, device=rank)
                    ...
                    val_step_loss, bd = criterion(        # ← this is inside the else: !
                        pred_int, gt_int, pred_glob, gt_glob,
                        z_mean, z_logvar, gt_latent, pred_fp,
                        identity_ids, ema_extractor, current_epoch=itr,
                        latent_weight=latent_weight, id_weight=id_weight,
                        collapse_weight=collapse_weight,   # ← collapse_weight never defined
                        max_epochs=max_iterations,
                        mode="global_only" if PHASE == "global_only" else "full"
                    )
